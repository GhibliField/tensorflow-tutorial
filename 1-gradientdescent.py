...
#构建tf结构
Weights=tf.Variable(tf.random_uniform([1],-0.1,1.0))#在当前场景下，权重的shape是1个数，故使用均匀分布的随机数生成函数初始化函数的时候定义为[1]，随机取值的空间在（-1,1）
biases=tf.Variable(tf.zeros([1]))#偏置项通常会使用常数来设置初始化值。这里直接初始为0
